...
import { FormAttributes, MultiGridForm } from "../types";
...
export default function MultiGrid({
  ...
  if (!form[tab] || form[tab].type !== "multigrid") {
    return null;
  }

  const currentForm = form[tab] as MultiGridForm;
            ...
            {currentForm.rows?.map((r, i) => (
              <div key={i} className='flex gap-3 bg-accent h-10 rounded-r-lg'>
                <RadioGroup
                  ...
                  onValueChange={(c) =>
                    onGridRadioChange(c, r.rowId, form, setForm, tab)
                  }
                  value={
                    currentForm.answer?.find((a) => a.rowId === r.rowId)
                      ?.columnId
                  }
                >
                  {currentForm.columns?.map((c, j) => (
                    <div key={j} className='flex items-center gap-3'>
                      <RadioGroupItem value={c.columnId} />
                    </div>
                  ))}
                </RadioGroup>
              </div>
...
const onGridRadioChange = (
  ...
  const isMultiGridForm = (
    form: FormAttributes["form"][number]
  ): form is MultiGridForm => {
    return form.type === "multigrid";
  };

  if (!currentForm || !isMultiGridForm(currentForm)) {
    return;
  }
  ...
  const isExist = currentForm.answer.find((x) => x.rowId === rowId);
  if (isExist) {
    const filtered = currentForm.answer.filter((x) => x.rowId !== rowId);
    filtered.push({ columnId, rowId });
